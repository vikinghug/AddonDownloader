...
function VikingGroupDisplay:OnGroupAdd(strMemberName) -- Someone else joined my group
  local strMsg = String_GetWeaselString(Apollo.GetString("GroupJoin"), strMemberName)
  self:AddToQueue(ktMessageIcon.Accept, strMsg)
  self:OnGroupUpdated()
end

function VikingGroupDisplay:OnGroupJoin() -- I joined a group
  self:OnGroupUpdated()
...
function VikingGroupDisplay:OnGroupRemove(strMemberName, eReason) -- someone else left the group
...
function VikingGroupDisplay:OnGroupMemberPromoted(strMemberName, bSelf) -- I've been promoted
  if bSelf then
...
function VikingGroupDisplay:OnGroupOperationResult(strMemberName, eResult)
...
function VikingGroupDisplay:OnGroupAcceptInvite() -- I've accepted an invitation
  self.wndGroupInviteDialog:Show(false)
end

function VikingGroupDisplay:OnGroupDeclineInvite() -- I've declined an invitation
  self.wndGroupInviteDialog:Show(false)
end
...
function VikingGroupDisplay:OnGroupMemberFlags(nMemberIndex, bIsFromPromotion, tChangedFlags)
...
function VikingGroupDisplay:OnGroupReadyCheck(nMemberIndex, strMessage)
  local tMember = GroupLib.GetGroupMember(nMemberIndex)
...
function VikingGroupDisplay:OnGroupInviteResult(strCharacterName, eResult)
...
function VikingGroupDisplay:CloseGroupHUD() -- see if the HUD can be closed
  if GroupLib.InGroup() and GroupLib.GetMemberCount() > 0 then
    return false
  end
...
-- Message Queue
---------------------------------------------------------------------------------------------------
function VikingGroupDisplay:AddToQueue(nMessageIcon, strMessageText)
...
function VikingGroupDisplay:ProcessAlerts()
  self:ClearFields()
...
function VikingGroupDisplay:DisplayAlert(nMessageIcon, strMessageText)
...
---------------------------------------------------------------------------------------------------
function VikingGroupDisplay:OnGroupMessageTimer()
  self.wndGroupMessage:FindChild("MessageBirthAnimation"):ToFront()
  self.wndGroupMessage:FindChild("MessageBirthAnimation"):SetSprite("sprWinAnim_BirthSmallTemp")
...
---------------------------------------------------------------------------------------------------
function VikingGroupDisplay:ClearFields() --clear everything
  for idx =1, #karMessageIconString do
    self.wndGroupMessage:FindChild(karMessageIconString[idx]):Show(false)
  end
...
function VikingGroupDisplay:OnGroupWrongInstance()
...
function VikingGroupDisplay:HelperResizeGroupContents()
  ...
  self.wndGroupHud:SetAnchorOffsets(nLeft, nTop, nRight, nTop + nOnGoingHeight * 2) -- TODO Hard coded formatting
end
...
function VikingGroupDisplay:OnGroupMentor(tMemberList, bCurrentlyMentoring, bUpdateOnly)
...
function VikingGroupDisplay:OnToggleMentorItem(wndHandler, wndCtrl)
...
function VikingGroupDisplay:OnMentorPlayerBtn(wndHandler, wndCtrl)
...
function VikingGroupDisplay:OnCancelMentoringBtn(wndHandler, wndCtrl)
...
function VikingGroupDisplay:OnMentorTimer()
  ...
  if self.fMentorTimerDiff <= knMentorTimeout then
    ...
    Event_FireGenericEvent("GenericEvent_SystemChannelMessage", Apollo.GetString("VikingGroupDisplay_MentorWindowTimedOut"))
    self:OnMentorCloseBtn()
  end
end

function VikingGroupDisplay:OnGroupMentorLeftAOI(nTimeUntilMentoringDisabled, bClearUI)
  if bClearUI then
...
function VikingGroupDisplay:OnMentorAOICloseBtn()
  self.wndMentorAOI:Show(false)
  Apollo.StopTimer("MentorAOITimer")
...
function VikingGroupDisplay:OnMentorAOITimer()
  -- This is the timer that's shown on the window
...
function VikingGroupDisplay:OnAcceptRequest()
  self.wndRequest:Show(false)
  GroupLib.AcceptRequest()
end

function VikingGroupDisplay:OnDenyRequest()
  self.wndRequest:Show(false)
...
function VikingGroupDisplay:OnRaidOkay( wndHandler, wndControl, eMouseButton )
  if self.wndRaidNotice and self.wndRaidNotice:IsValid() then
...
---------------------------------------------------------------------------------------------------
local GroupFrameInst = VikingGroupDisplay:new()
VikingGroupDisplay:Init()

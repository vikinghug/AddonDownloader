...
local VikingGroupDisplayOptions = {}
...
function VikingGroupDisplayOptions:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
    return o
...
function VikingGroupDisplayOptions:Init()
...
function VikingGroupDisplayOptions:Initialize(wndParent)
...
function VikingGroupDisplayOptions:OnSubOptionSet(wndHandler, wndControl)
...
function VikingGroupDisplayOptions:RedrawAll() -- The button check and various UI calls route here
...
function VikingGroupDisplayOptions:HelperRedrawSpecific(strMenuName, tProvidedValue)
...
function VikingGroupDisplayOptions:HelperRedrawMentoring() -- Also from XML's Mentoring Btn
...
function VikingGroupDisplayOptions:OnMentorSpecificPerson(wndHandler, wndControl)
...
function VikingGroupDisplayOptions:OnStopMentoringBtn(wndHandler, wndControl)
...
function VikingGroupDisplayOptions:OnLeaveGroup()
	self.wndMain:Show(false)
	self:OnGroupFormatClose()
	Event_FireGenericEvent("GenericEvent_ShowConfirmLeaveDisband", 1)
end

function VikingGroupDisplayOptions:OnDisbandGroup()
	self.wndMain:Show(false)
	self:OnGroupFormatClose()
	Event_FireGenericEvent("GenericEvent_ShowConfirmLeaveDisband", 0)
end

function VikingGroupDisplayOptions:OnResetInstances()
	self.wndMain:Show(false)
	self:OnGroupFormatClose()
	Apollo.ParseInput("/resetinstances") -- TODO: Replace with a library call
end

function VikingGroupDisplayOptions:OnConvertToRaid()
...
function VikingGroupDisplayOptions:OnGroupFormatClose() -- clean up on close
	for idx, strMenu in ipairs(ktSubMenus) do
		if strMenu and self.wndMain:FindChild(strMenu) then
...
function VikingGroupDisplayOptions:OnRaidConfirmNo(wndHandler, wndControl, eMouseButton)
	if self.wndRaidConfirm and self.wndRaidConfirm:IsValid() then
		self.wndRaidConfirm:Destroy()
		self.wndRaidConfirm = nil
	end
...
function VikingGroupDisplayOptions:OnRaidConfirmYes(wndHandler, wndControl, eMouseButton)
	if self.wndRaidConfirm and self.wndRaidConfirm:IsValid() then
		self.wndRaidConfirm:Destroy()
		self.wndRaidConfirm = nil
	end
...
function VikingGroupDisplayOptions:HelperInitializeWindows(tLootThreshold, strLuaNameId, wndParent)
